...
rag_agent = Agent(
    ...
    retries=2, 
    system_prompt=("You are an expert in rabbit races and knows how to distinguish between the rabbits",
...
def retrieve_top_documents(query: str, top_results = 3) -> str:
    """Uses vector search to find top results closest matching documents to use as context to the query"""
    result = vector_db["articles"].search(query=query).limit(top_results).to_list()
