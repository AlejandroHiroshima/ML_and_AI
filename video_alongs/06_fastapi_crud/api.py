...
async def filter_books(
    start_year:int = Query(1950, gt=1500, lt=CURRENT_YEAR +1,
                           description="Filters books that are newer than this year"),
    author: str =Query(None, description = "Filters authors firstname and lastname")):
    filtered_books = [book for book in books if start_year < book.year]

    if author:
        filtered_books = [book for book in books if author.casefold() == book.author.casefold()]
